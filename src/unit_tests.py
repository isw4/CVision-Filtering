...
def test_fft_blur(image, filter):
	...
	shifted = np.ndarray((im_row, im_col, im_height), dtype="complex128")
	...
	for i in range(0, im_height):
		...
		shifted[:,:,i] = fft.ifft2(mult_fft[:,:,i])
...
def main():
	...
	shifted, low = test_fft_blur(image1, filter)
...
image = np.array([[2, 3, 3], [3, 5, 5], [4, 4, 6]])
sum = np.sum(filter*image)
print(sum)